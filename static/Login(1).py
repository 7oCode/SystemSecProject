...
def card():
    ...
    if regcard.validate_on_submit():
        print(regcard.validate_on_submit())
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        fullname = f"{regcard.fname.data} {regcard.lname.data}"
        cursor.execute("INSERT INTO card_info VALUES (NULL, %s, %s, %s, %s)", (fullname, regcard.card_num.data, regcard.exp_date.data,regcard.cvv.data,))
        print('Card added')
